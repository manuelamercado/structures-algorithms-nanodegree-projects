...
def calc_distance(node, goal):
    dist = math.sqrt((goal[0] - node[0])**2 + (goal[1] - node[1])**2)
    
    return dist

def get_next_neighbor(M, current_intersection, neighbor, goal):
    index = current_intersection
    dist_traveled = 0

    if current_intersection != neighbor:
        dist_traveled = current_intersection[1]["g_cost"]
        index = current_intersection[1]["index"]

    g_cost = calc_distance(M.get("intersections").get(index), M.get("intersections").get(neighbor))
    h_cost = dist_traveled + abs(M.get("intersections").get(neighbor)[0]-M.get("intersections").get(goal)[0])+abs(M.get("intersections").get(neighbor)[1]-M.get("intersections").get(goal)[1])
...
def shortest_path(M,start,goal):
    ...
    neighbor = get_next_neighbor(M, current_intersection, current_intersection, goal)
    heapq.heappush(costs, neighbor)
    frontier.append(current_intersection)

    while not is_finished and len(costs) > 0:
        current_value = heapq.heappop(costs)
        current_index = current_value[1]["index"]
        ...
        neighbors = M.get("roads")[current_index]

        for neighbor_index in neighbors:
            if neighbor_index not in visited:
                neighbor = get_next_neighbor(M, current_value, neighbor_index, goal)
                if neighbor_index not in frontier:
                    heapq.heappush(costs, neighbor)
                    frontier.append(neighbor_index)
            else:
                if neighbor_index in visited and neighbor_index in frontier and neighbor_index != start:
                    visited.remove(neighbor_index)
